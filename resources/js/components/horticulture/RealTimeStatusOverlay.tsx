...
const RealTimeStatusOverlay: React.FC<RealTimeStatusOverlayProps> = ({
    projectState,
    t,
}) => {
    ...
    return (
        <div className="absolute right-4 top-16 rounded-lg border border-gray-200 bg-gray-900 p-3 shadow-lg max-w-xs">
            ...
            <div className="">
                ...
                {completedSteps === totalSteps && (
                 ...
                 <p className="text-[12px] text-gray-400 my-1">หมายเหตุ: ไม่จำเป็นต้องทำทุกขั้นตอน</p>
            </div>

            {/* Collapsible content */}
            {!isCollapsed && (
                <>
                    <div className="mb-1 text-xs text-blue-300 font-semibold">Tab: พื้นที่</div>
                    <ol className="list-decimal ml-5 space-y-1">
                        {steps.slice(0, 5).map((step) => (
                            <li key={step.id} className="text-xs text-white flex items-center justify-between group hover:bg-gray-800 rounded px-1 py-0.5 transition-colors">
                                <span className="flex-1">
                                    {step.title}{' '}
                                    {step.required && <span className="text-yellow-400">{t('*')}</span>}
                                </span>
                                {step.isCompleted ? (
                                    <span className="ml-2 text-green-400 flex-shrink-0" title="เสร็จแล้ว">
                                        <svg className="w-4 h-4 animate-pulse" fill="currentColor" viewBox="0 0 20 20">
                                            <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                                        </svg>
                                    </span>
                                ) : step.required ? (
                                    <span className="ml-2 text-yellow-400 flex-shrink-0" title="รอทำ">
                                        <svg className="w-4 h-4 animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                                        </svg>
                                    </span>
                                ) : (
                                    <span className="ml-2 text-gray-400 flex-shrink-0" title="ไม่จำเป็น">
                                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="mb-1 mt-2 text-xs text-blue-300 font-semibold">Tab: ระบบน้ำ</div>
                    ...
                    <div className="mb-1 mt-2 text-xs text-blue-300 font-semibold">Tab: แก้ไข</div>
                    <ol className="list-decimal ml-5 space-y-1" start={9}>
                    ...
                    <div className="mb-1 mt-2 text-xs text-blue-300 font-semibold">Tab: สรุป</div>
                    <ol className="list-decimal ml-5 space-y-1" start={10}>
                    ...
                    <div className="text-xs mt-2 text-white border-t border-gray-700 pt-2">
                        <p>เครื่องหมาย <span className="text-yellow-400">*</span> คือขั้นตอนที่จำเป็นที่ต้องทำ</p>
