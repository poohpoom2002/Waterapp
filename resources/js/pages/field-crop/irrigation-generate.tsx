...
export default function Step4_IrrigationSystem() {
    ...
    useEffect(() => {
        ...
        new google.maps.Polygon({
            paths: projectData.mainField.coordinates,
            map,
            fillColor: '#22C55E',
            fillOpacity: 0.1,
            strokeColor: '#22C55E',
        ...
        projectData.equipmentIcons.forEach((eq: any) => {
            ...
            new google.maps.Marker({
                ...
                title: eq.name,
                icon: {
                    url: `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="${config.color}" width="32px" height="32px"><text x="12" y="18" font-size="20" text-anchor="middle">${config.icon}</text></svg>`)}`,
    ...
    const finishProject = () => {
        ...
        const dataToSave = {
            ...
            irrigationRadius,
            sprinklerOverlap,
            irrigationPoints: irrigationPoints.map((p) => ({
                id: p.id,
                lat: p.lat,
                lng: p.lng,
                type: p.type,
                                        ...
                                        <div className="mt-2 space-y-2 text-xs">
                                            <div>
                                                ...
                                                <select
                                                    ...
                                                    className="mt-1 w-full rounded bg-gray-700 p-1"
                                                >
                                            ...
                                            {irrigationAssignments[zone.id.toString()] ===
                                                ...
                                                <>
                                                    <div>
                                                        <label>
                                                            รัศมี (เมตร):{' '}
                                                            {irrigationRadius[zone.id.toString()] ||
                                                                8}
                                                        </label>
                                                        <input
                                                            type="range"
                                            ...
                                            <div className="mt-2 flex gap-2">
                                                <button
                                                    onClick={() =>
                                                        generateIrrigationForZone(zone.id)
                                                    }
                                                    className="flex-1 rounded bg-green-600 px-3 py-1 hover:bg-green-700"
                                                >
                                                    สร้าง
                                                </button>
                                                <button
                                                    onClick={() => clearIrrigationForZone(zone.id)}
                                                    className="flex-1 rounded bg-red-600 px-3 py-1 hover:bg-red-700"
                                                >
                                                    ล้าง
                                                </button>
                                            </div>
