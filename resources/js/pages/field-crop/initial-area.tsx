...
export default function InitialArea({
    ...
    const createPlantMarkers = useCallback(
        async (points: PlantPoint[], generationId: number) => {
    ...
    const generatePlantPointsOriented = useCallback(
        async (angleDeg: number, generationId: number): Promise<PlantPoint[]> => {
            ...
            const rotatedObstacles = obstaclesXY.map((poly) =>
                poly.map((p) => rotateXY(p, -angleRad))
            );
            ...
            log(
                `Plant generation: center Y=${centerY.toFixed(2)}, rows above=${rowsAboveCenter}, rows below=${rowsBelowCenter}`
            );
            ...
            for (let rowIndex = 0; rowIndex < allRowYs.length; rowIndex++) {
                ...
                for (let x = minX; x <= maxX; x += plantSpacingM) {
                    ...
                    if (plantIndex % 50 === 0 && currentGenerationIdRef.current !== generationId) {
                        log(
                            `Plant generation ${generationId} cancelled at plant ${plantIndex} in row ${rowIndex}`
                        );
                        return [];
                    }
            ...
            log(
                `Plant generation ${generationId} completed with ${plantPointsOut.length} points using center-first approach`
            );
    ...
    const runPlantGeneration = useCallback(
        async (angle: number) => {
            ...
            try {
                ...
                if (currentGenerationIdRef.current === generationId) {
                    ...
                    log(
                        `Generation ${generationId} completed successfully with ${markers.length} markers`
                    );
                } else {
                    log(
                        `Generation ${generationId} was cancelled during marker creation, cleaning up`
                    );
                    markers.forEach((m) => m.setMap(null));
                }
            } catch (error) {
                ...
                if (error instanceof Error && error.message === 'Plant generation timeout') {
                    alert(
                        t(
                            'Plant generation took too long. Please try with a smaller area or different spacing.'
                        )
                    );
                } else {
                    alert(
                        t(
                            'Error generating plant points. Please check your area and spacing settings.'
                        )
                    );
                }
    ...
    useEffect(() => {
        ...
        if (hasUrlParams) {
            try {
                ...
                if (fieldDataStr) {
                    ...
                    if (
                        fieldData.rotationAngle !== null &&
                        fieldData.rotationAngle !== undefined &&
                        !isNaN(fieldData.rotationAngle)
                    ) {
        ...
        if (mainAreaData) {
            try {
                ...
                errorLog('Error parsing mainAreaData:', e);
            }
        }
        ...
        if (obstaclesData) {
            try {
                ...
                errorLog('Error parsing obstaclesData:', e);
            }
        }
        ...
        if (plantPointsData) {
            try {
                ...
                errorLog('Error parsing plantPointsData:', e);
            }
        }
        ...
        if (rotationAngleData) {
            setRotationAngle(parseFloat(rotationAngleData));
        }
        ...
        if (rowSpacingData) {
            try {
                ...
                errorLog('Error parsing rowSpacingData:', e);
            }
        }
        ...
        if (plantSpacingData) {
            try {
                ...
                errorLog('Error parsing plantSpacingData:', e);
            }
        }
    }, [
        crops,
    ...
    const navigateToStep = (step: StepData) => {
        ...
        try {
            const fieldData = {
                ...
                plantPoints: plantPoints.filter(
                    (point) =>
                        point &&
                        typeof point.lat === 'number' &&
                        typeof point.lng === 'number' &&
                ...
                perimeterMeters:
                    typeof perimeterMeters === 'number' && !isNaN(perimeterMeters)
                        ...
                        : null,
                rotationAngle:
                    typeof rotationAngle === 'number' && !isNaN(rotationAngle) ? rotationAngle : 0,
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            {!isMainAreaSet && (
                                                <div className="rounded bg-orange-900 bg-opacity-30 p-2 text-xs text-orange-300">
                                                    🔒{' '}
                                                    {t(
                                                        'Please set main area before generating plants'
                                                    )}
                                                </div>
                                            )}
                                            <div className="flex items-center justify-between text-xs">
                                            ...
                                            {plantCount > 0 && (
                                                <div className="rounded bg-blue-900 bg-opacity-30 p-2 text-xs text-blue-300">
                                                    💡{' '}
                                                    {t(
                                                        'Points are placed with 30% buffer from edges for optimal growth'
                                                    )}
                                                </div>
                                            )}
                                            {plantCount === 0 && (
                            ...
                            <div className="border-t border-white p-4">
                                <div className="flex gap-2">
                                    ...
                                    <button
                                        onClick={() => {
                                            log(
                                                'Resetting initial area - clearing drawn areas but keeping crops'
                                            );
                                            setMainArea([]);
                                            setAreaRai(null);
                                            setPerimeterMeters(null);
                        ...
                        <div className="relative flex-1">
                            ...
                            >
                                ...
                                <div className="pointer-events-none absolute bottom-4 right-20 z-10">
                                    <div className="rounded border border-white bg-black bg-opacity-70 px-2 py-1 text-xs text-white">
