    ...
    if (coordinates.length > targetCount * 2) {
        try {
            ...
            if (
                ...
                simplified.geometry &&
                simplified.geometry.type === 'LineString'
            ) {
                const coords = (simplified.geometry as { coordinates: [number, number][] })
                    .coordinates;
...
const useWaterCalculations = (fieldData: FieldData) => {
    ...
    const calculateWaterPerPoint = useCallback(
        (point: PlantPoint): number => {
            if (fieldData.plantPoints.length === 0 || fieldData.totalWaterRequirement === 0)
                return 0;
            const multiplier = getPlantMultiplier(point.cropType);
            const denom =
                totalWaterMultiplier > 0 ? totalWaterMultiplier : fieldData.plantPoints.length;
            return (fieldData.totalWaterRequirement * multiplier) / denom;
        },
        [
            fieldData.plantPoints.length,
            fieldData.totalWaterRequirement,
            totalWaterMultiplier,
            getPlantMultiplier,
        ]
    );
...
export default function ZoneObstacle(props: ZoneObstacleProps) {
    ...
    const initialFieldData = useMemo((): FieldData => {
        ...
        const localStorageData = parseJsonSafely(
            localStorage.getItem('fieldCropData') || '',
        ...
        if (!hasUrlParams) {
            ...
            if (Object.keys(localStorageData).length > 0) {
                return {
                    ...
                    irrigationCounts: localStorageData.irrigationCounts || {
                    ...
                    irrigationPositions: localStorageData.irrigationPositions || {
        ...
        return {
            ...
            zones: localStorageData.zones || [],
            selectedCrops: props.crops
                ? props.crops.split(',').filter((c) => c.trim())
                : localStorageData.selectedCrops || [],
            selectedIrrigationType:
                props.selectedIrrigationType || localStorageData.selectedIrrigationType || '',
            irrigationCounts: parseJsonSafely(
                props.irrigationCounts,
                localStorageData.irrigationCounts || { ...DEFAULT_IRRIGATION_COUNTS }
            ),
            totalWaterRequirement:
                parseFloat(props.totalWaterRequirement || '') ||
                localStorageData.totalWaterRequirement ||
                0,
            irrigationSettings: parseJsonSafely(
                props.irrigationSettings,
                localStorageData.irrigationSettings || {}
            ),
            irrigationPositions: parseJsonSafely(
                props.irrigationPositions,
                localStorageData.irrigationPositions || { ...DEFAULT_IRRIGATION_POSITIONS }
            ),
            areaRai: parseFloat(props.areaRai || '') || localStorageData.areaRai || null,
            perimeterMeters:
                parseFloat(props.perimeterMeters || '') || localStorageData.perimeterMeters || null,
            rotationAngle:
                parseFloat(props.rotationAngle || '') || localStorageData.rotationAngle || 0,
    ...
    const calculateZoneWaterInfo = useCallback(
        (coordinates: Coordinate[]) => {
            const plantsInZone = fieldData.plantPoints.filter((point) =>
                isPointInOrOnPolygon(point, coordinates)
            );
            const waterRequirement = plantsInZone.reduce(
                (sum, point) => sum + calculateWaterPerPoint(point),
                0
            );
            return {
                waterRequirement,
    ...
    const createCombinedPoints = useCallback((): CombinedPoint[] => {
        ...
        const irrigationScale =
            totalIrrigationFlow > 0 ? totalPlantWeight / Math.max(totalIrrigationFlow, 1) : 1;
        ...
        fieldData.irrigationPositions.sprinklers.forEach((sprinkler, index) => {
            const sprinklerWeight = Math.max(
                flowPerSprinkler * irrigationScale,
                MIN_WEIGHTS.sprinkler
            );
            combinedPoints.push({
                id: `sprinkler-${index}`,
                lat: sprinkler.lat,
        ...
        fieldData.irrigationPositions.dripTapes.forEach((dripTape, index) => {
            const dripTapeWeight = Math.max(
                flowPerDripTape * irrigationScale,
                MIN_WEIGHTS.dripTape
            );
            combinedPoints.push({
                id: `dripTape-${index}`,
                lat: dripTape.lat,
        ...
        fieldData.irrigationPositions.waterJets.forEach((waterJet, index) => {
            const waterJetWeight = Math.max(
                flowPerWaterJet * irrigationScale,
                MIN_WEIGHTS.waterJet
            );
            combinedPoints.push({
                id: `waterJet-${index}`,
                lat: waterJet.lat,
                                                    ...
                                                    <>
                                                        <div className="mt-2 border-t border-gray-600 pt-2">
                                                            ...
                                                            <div className="flex justify-between text-gray-400">
                                                                ...
                                                                <span className="text-green-400">
                                                                    {
                                                                        plantCoverageStats.coveredPlants
                                                                    }{' '}
                                                                    {t('points')}
                                                                </span>
                                                            </div>
                                                            <div className="flex justify-between text-gray-400">
                                                                ...
                                                                <span className="text-yellow-400">
                                                                    {
                                                                        plantCoverageStats.borderPlants
                                                                    }{' '}
                                                                    {t('points')}
                                                                </span>
                                                            </div>
                                                            <div className="flex justify-between text-gray-400">
                                                                ...
                                                                <span className="text-red-400">
                                                                    {
                                                                        plantCoverageStats.uncoveredPlants
                                                                    }{' '}
                                                                    {t('points')}
                                                                </span>
                                                            </div>
                                                            <div className="flex justify-between text-gray-400">
                                                                ...
                                                                <span
                                                                    className={`font-semibold ${
                                                                        plantCoverageStats.coveragePercentage >=
                                                                        90
                                                                            ? 'text-green-400'
                                                                            : plantCoverageStats.coveragePercentage >=
                                                                                70
                                                                ...
                                                                >
                                                                    {plantCoverageStats.coveragePercentage.toFixed(
                                                                        1
                                                                    )}
                                                                    %
                                                                </span>
                                    ...
                                    {fieldData.selectedIrrigationType && (
                                        <div className="rounded-lg border border-white p-4">
                                            ...
                                            <div className="space-y-2 text-xs">
                                                ...
                                                {Object.entries(fieldData.irrigationCounts).map(
                                                    ([type, count]) =>
                                                        count > 0 &&
                                    ...
                                    {zoneStats && fieldData.zones.length > 0 && (
                                        <div className="rounded-lg border border-white p-4">
                                            ...
                                            <div className="space-y-2 text-xs">
                                                ...
                                                {zoneStats.balanceScore !== null &&
                                                    zoneStats.balanceScore !== undefined &&
                                                    zoneStats.balanceStatus && (
                                                        <>
                                                            <div className="flex justify-between text-gray-400">
                                                                ...
                                                                <span
                                                                    className={`font-semibold ${
                                                                        zoneStats.balanceScore >= 80
                                                                            ? 'text-green-400'
                                                                            : zoneStats.balanceScore >=
                                                                                60
                                                                              ? 'text-blue-400'
                                                                              : zoneStats.balanceScore >=
                                                                                  40
                                                                ...
                                                                >
                                                                    {zoneStats.balanceScore.toFixed(
                                                                        1
                                                                    )}
                                                                    %
                                                                </span>
                                                    ...
                                                    {fieldData.zones.map((zone, index) => (
                                                        ...
                                                        >
                                                            ...
                                                            <div className="space-y-1 text-xs">
                                                                ...
                                                                {(() => {
                                                                    const allPlantsInZone =
                                                                        fieldData.plantPoints.filter(
                                                                            (point) =>
