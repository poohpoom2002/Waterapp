        ...
        <div className="absolute left-[60px] top-4 z-[1000] w-80">
            <div className="relative">
                ...
                {suggestions.length > 0 && (
                    <ul className="absolute w-full rounded-b-lg bg-white shadow-lg max-h-60 overflow-y-auto">
                        {suggestions.map((item) => (
                            <li
                                key={item.place_id}
                                onClick={() => handleSuggestionClick(parseFloat(item.lat), parseFloat(item.lon), item.display_name)}
...
export default function HomeGardenPlanner() {
    ...
    const generatePipeNetwork = useCallback(async () => {
        ...
        router.post('/home-garden/generate-sprinkler', {
            area: area,
    ...
    const areaInSquareMeters = area.length > 0 ? L.GeometryUtil.geodesicArea(area.map(c => new L.LatLng(c.lat, c.lng))) : 0;
    const areaInRai = areaInSquareMeters / 1600;
    const estimatedSprinklers = areaInSquareMeters > 0 ? Math.ceil(areaInSquareMeters / (Math.PI * Math.pow(sprinklerRadius, 2) * 0.8)) : 0;

    return (
        <div className="min-h-screen bg-gray-900 p-6 text-white">
            <div className="mx-auto max-w-screen-xl">
                <h1 className="mb-4 text-3xl font-bold">🏡 Home Garden Sprinkler Calculator</h1>
                <p className="mb-6 text-gray-400">วาดพื้นที่สวน → กำหนดรัศมี Sprinkler → ดูผลการคำนวณ</p>
                ...
                <div className="grid grid-cols-1 gap-6 lg:grid-cols-4">
                    <div className="space-y-6 lg:col-span-1">
                        <div className="rounded-lg bg-gray-800 p-4">
                            ...
                            <div className="space-y-3 text-sm text-gray-300">
                                <div className="flex justify-between"><span>ขนาดพื้นที่ (ตร.ม.):</span><span className="font-medium text-white">{areaInSquareMeters.toFixed(2)}</span></div>
                                <div className="flex justify-between"><span>ขนาดพื้นที่ (ไร่):</span><span className="font-medium text-white">{areaInRai.toFixed(3)}</span></div>
                                <div className="flex justify-between"><span>จุดขอบเขต:</span><span className="font-medium text-white">{area.length} จุด</span></div>
                            </div>
                        </div>
                        <div className="rounded-lg bg-gray-800 p-4">
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className="mb-2 block text-sm font-medium text-gray-300">รัศมีการฉีดน้ำ (เมตร)</label>
                                    <input type="range" min="0.5" max="20" step="0.5" value={sprinklerRadius} onChange={(e) => setSprinklerRadius(Number(e.target.value))} className="w-full h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer" />
                                    <div className="mt-2 flex justify-between text-xs text-gray-400">
                                        <span>0.5m</span>
                                        <span className="font-bold text-xl text-blue-400">{sprinklerRadius}m</span>
                                        <span>20m</span>
                                    </div>
                                </div>
                                <div className="rounded bg-blue-900/30 p-3">
                                    <div className="text-sm text-blue-300">
                                        <div className="flex justify-between"><span>พื้นที่คลุม/หัว:</span><span className="font-medium">{(Math.PI * Math.pow(sprinklerRadius, 2)).toFixed(1)} ตร.ม.</span></div>
                                        <div className="flex justify-between"><span>ประมาณการ:</span><span className="font-bold text-blue-200">~{estimatedSprinklers} หัว</span></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="flex justify-center">
                            <button onClick={handleCalculate} disabled={area.length === 0} className="w-full rounded-lg bg-green-600 px-8 py-3 text-lg font-semibold text-white transition-transform duration-200 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-green-500 disabled:bg-gray-600 disabled:cursor-not-allowed disabled:transform-none">
                                🧮 คำนวณ Sprinkler
                            </button>
                        </div>
                    </div>
                    <div className="lg:col-span-3">
                        <div className="h-[700px] w-full overflow-hidden rounded-lg border border-gray-700">
                            <MapContainer center={mapCenter} zoom={16} maxZoom={22} style={{ height: '100%', width: '100%' }}>
                                ...
                                <FeatureGroup ref={featureGroupRef}>
                                    <EditControl position="topleft" onCreated={onCreated} onDeleted={onDeleted} onEdited={onEdited} draw={{ rectangle: true, circle: false, circlemarker: false, marker: false, polyline: false, polygon: true }}/>
                                </FeatureGroup>
                            </MapContainer>
                        </div>
                    </div>
