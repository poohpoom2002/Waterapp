    ...
    const calculateLateralPipeFlow = () => {
        ...
        lateralPipes.forEach((lateral) => {
            ...
            if (lateral.plants && lateral.plants.length > 0) {
                ...
                zoneId =
                    firstPlant.zoneId || findZoneFromCoordinate(firstPlant.position) || 'no-zone';
            ...
            subMainByZone.get(zoneId).push({
                ...
                branchCount:
                    ...
                    lateralPipes.filter((l) => {
                        if (l.subMainPipeId !== subMain.id || !l.plants || l.plants.length === 0)
                            return false;
                        const lateralZone =
                            l.plants[0].zoneId ||
...
export default function EnhancedHorticulturePlannerPage() {
    ...
    useEffect(() => {
        ...
        const hasChanges =
            JSON.stringify(currentState.mainArea) !==
                JSON.stringify(prevStateRef.current.mainArea) ||
            JSON.stringify(currentState.zones) !== JSON.stringify(prevStateRef.current.zones) ||
            JSON.stringify(currentState.exclusionAreas) !==
                JSON.stringify(prevStateRef.current.exclusionAreas) ||
            JSON.stringify(currentState.pump) !== JSON.stringify(prevStateRef.current.pump) ||
            JSON.stringify(currentState.mainPipes) !==
                JSON.stringify(prevStateRef.current.mainPipes) ||
            JSON.stringify(currentState.subMainPipes) !==
                JSON.stringify(prevStateRef.current.subMainPipes) ||
                ...
                if (
                    ...
                    try {
                        ...
                        if (closestPlant && minDistance <= 25) {
                            const direction = getDragOrientation(
                                connectionPoint,
                            ...
                            const sameRowPlants = history.present.plants.filter((plant) => {
                                if (direction === 'columns') {
                                    return (
                                        Math.abs(plant.position.lng - closestPlant.position.lng) <
                                        0.00002
                                    );
                                } else {
                                    return (
                                        Math.abs(plant.position.lat - closestPlant.position.lat) <
                                        0.00002
                                    );
                                }
                            });
                            ...
                            if (sameRowPlants.length > 0) {
                                ...
                                let maxDistance = calculateDistanceBetweenPoints(
                                    connectionPoint,
                                    farthestPlant.position
                                );
                            ...
                            if (
                                snappedStartPoint.lat === connectionPoint.lat &&
                                snappedStartPoint.lng === connectionPoint.lng
                            ) {
