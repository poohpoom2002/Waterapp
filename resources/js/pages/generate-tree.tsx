...
function findNearestRowPoint(lat: number, lng: number, grid: LatLng[][] | null): [number, number] {
    ...
    for (let rowIndex = 0; rowIndex < grid.length; rowIndex++) {
        ...
        for (let colIndex = 0; colIndex < row.length; colIndex++) {
            const point = row[colIndex];
            if (
                Math.abs(point.lat - nearestPoint.lat) < 0.0001 &&
    ...
    const lngOffsetMeters =
        ((offsetLng * R * Math.PI) / 180) * Math.cos((nearestPoint.lat * Math.PI) / 180);
    ...
    const snappedLngOffset =
        snappedLngOffsetMeters /
        (((R * Math.PI) / 180) * Math.cos((nearestPoint.lat * Math.PI) / 180));
...
const PointManagementControls = ({
    ...
    setMovingPoints: React.Dispatch<
        React.SetStateAction<
            { id: string; originalPosition: [number, number]; currentPosition: [number, number] }[]
        >
    >;
    editMode: 'select' | 'add' | 'delete' | null;
    ...
    const handleMapClick = (e: LeafletMouseEvent) => {
        if (editMode === 'add' && !isAddingWithPolygon) {
            // Only handle clicks when not in polygon mode
            // Find the nearest point from the original grid structure
            const [nearestLat, nearestLng] = findNearestRowPoint(e.latlng.lat, e.latlng.lng, grid);

            // Only add if the point is inside the area and not already added
            if (
                isPointInPolygon(nearestLat, nearestLng, area) &&
    ...
    const handleMouseMove = (position: [number, number]) => {
        if (movingPoints.length > 0) {
            ...
            const centerLat =
                movingPoints.reduce((sum, mp) => sum + mp.originalPosition[0], 0) /
                movingPoints.length;
            const centerLng =
                movingPoints.reduce((sum, mp) => sum + mp.originalPosition[1], 0) /
                movingPoints.length;
            ...
            <div className="absolute left-[60px] top-4 z-[1000] rounded-lg bg-white p-2 shadow-lg">
                <div className="flex flex-col space-y-2">
                    ...
                    <div className="flex space-x-2">
                        ...
                        >
                            {isDeletingWithPolygon
                                ? 'Draw Polygon to Delete Points'
                                : 'Delete Points'}
                        </button>
                        <button
            ...
            <FeatureGroup ref={featureGroupRef}>
                <EditControl
                    ...
                    draw={{
                        rectangle: false,
                        polygon:
                            isDeletingWithPolygon || isAddingWithPolygon || isMovingWithPolygon,
                        circle: false,
                        circlemarker: false,
                        marker: false,
                        polyline: false,
...
export default function GenerateTree({ areaType, area, plantType, layers = [] }: Props) {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const exclusionLayers = layers.filter(
                (layer) =>
                        ...
                        if (rowPoints.length >= 2) {
                            ...
                            newPipeLayout.push({
                                ...
                                length: calculateDistance(
                                    rowPoints[0].lat,
                                    rowPoints[0].lng,
                                    rowPoints[rowPoints.length - 1].lat,
                                    rowPoints[rowPoints.length - 1].lng
                                ),
                            });
                        }
                    });
                        ...
                        if (columnPoints.length >= 2) {
                            ...
                            newPipeLayout.push({
                                ...
                                length: calculateDistance(
                                    columnPoints[0].lat,
                                    columnPoints[0].lng,
                                    columnPoints[columnPoints.length - 1].lat,
                                    columnPoints[columnPoints.length - 1].lng
                                ),
                            });
                        }
                    }
    ...
    const longestMainPipe = userPipes
        .filter((p) => p.type === 'main')
        .find((p) => polylineLength(p.coordinates) === longestMain);
    ...
    const longestSubmainPipe = userPipes
        .filter((p) => p.type === 'submain')
        .find((p) => polylineLength(p.coordinates) === longestSubmain);
              ...
              for (let i = 0; i < longestSubmainPipe.coordinates.length - 1; i++) {
                  ...
                  if (
                      doLineSegmentsIntersect(
                          ...
                          submainEnd[0],
                          submainEnd[1],
                          branchPipeCoords[0][0],
                          branchPipeCoords[0][1],
                          branchPipeCoords[1][0],
                          branchPipeCoords[1][1]
                      )
                  ) {
                      return true;
                  }
    ...
    const zonePipeLengths = zones.map((zone) => {
        ...
        const longestPipeLength =
            zonePipes.length > 0 ? Math.max(...zonePipes.map((pipe) => pipe.length)) : 0;
            ...
            ? plantLocations.filter((plant) => {
                  ...
                  if (lenSq === 0) {
                      ...
                      const distance = Math.sqrt(
                          Math.pow(plant.lat - startLat, 2) + Math.pow(plant.lng - startLng, 2)
                      );
                  ...
                  const distance = Math.sqrt(
                      Math.pow(plant.lat - closestLat, 2) + Math.pow(plant.lng - closestLng, 2)
                  );
    ...
    const longestBranchPipeData = zonePipeLengths
        ...
        .reduce(
            (longest, zone) =>
                ...
                zone.longestBranchPipe.length > (longest?.longestBranchPipe?.length || 0)
                    ? zone
                    : longest,
            null as any
        );
    ...
    const zoneData = zones.map((zone) => {
        const pointsInZone = plantLocations.filter(
            (point) => zone.polygon && isPointInZonePolygon(point.lat, point.lng, zone.polygon)
        );
                    ...
                    longestSubmainPipe: {
                        ...
                        totalPlantsServed: longestSubmainPipe
                            ? plantLocations.filter((plant) => {
                                  ...
                                  for (
                                      let i = 0;
                                      i < longestSubmainPipe.coordinates.length - 1;
                                      i++
                                  ) {
        ...
        <div className="min-h-screen bg-gray-900 p-6">
            ...
            {showPipeSummary && (
                <div className="mt-4 rounded-lg bg-gray-800 p-4 text-white">
                    <h3 className="mb-4 text-lg font-semibold">Pipe Summary</h3>
                                        ...
                                        <span className="font-bold">
                                            {
                                                plantLocations.filter((plant) => {
                                                    ...
                                                    for (
                                                        let i = 0;
                                                        i < longestMainPipe.coordinates.length - 1;
                                                        i++
                                                    ) {
                                                        const startLat =
                                                            longestMainPipe.coordinates[i][0];
                                                        const startLng =
                                                            longestMainPipe.coordinates[i][1];
                                                        const endLat =
                                                            longestMainPipe.coordinates[i + 1][0];
                                                        const endLng =
                                                            longestMainPipe.coordinates[i + 1][1];
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-bold">
                                        {totalSubmainLength.toFixed(2)} m
                                    </span>
                                </div>
                                <div className="flex justify-between">
                                        ...
                                        <span className="font-bold">
                                            {
                                                plantLocations.filter((plant) => {
                                                    ...
                                                    for (
                                                        let i = 0;
                                                        i <
                                                        longestSubmainPipe.coordinates.length - 1;
                                                        i++
                                                    ) {
                                                        const startLat =
                                                            longestSubmainPipe.coordinates[i][0];
                                                        const startLng =
                                                            longestSubmainPipe.coordinates[i][1];
                                                        const endLat =
                                                            longestSubmainPipe.coordinates[
                                                                i + 1
                                                            ][0];
                                                        const endLng =
                                                            longestSubmainPipe.coordinates[
                    ...
                    <div className="mt-6">
                        <h4 className="text-md mb-4 font-medium text-gray-300">
                            Zone Pipe Details
                        </h4>
                        <div className="space-y-4">
                            {zonePipeLengths.map((zone) => (
                                <div key={zone.zoneId} className="rounded bg-gray-700 p-3">
                                    <div className="mb-2 flex items-center gap-2">
                                        <span
                                            style={{
                                                background: zones.find((z) => z.id === zone.zoneId)
                                                    ?.color,
                                                width: 12,
                                                height: 12,
                                                display: 'inline-block',
                                    ...
                                    <div className="grid grid-cols-2 gap-4 text-sm">
                                        <div className="flex justify-between">
                                            ...
                                            <span className="font-bold">
                                                {zone.longestPipe.toFixed(2)} m
                                            </span>
                                        </div>
                                        <div className="flex justify-between">
                                            <span>Total Branch Length:</span>
                                            <span className="font-bold">
                                                {zone.totalLength.toFixed(2)} m
                                            </span>
                                        </div>
                                        {zone.longestBranchPipe && (
                                            <>
                                                <div className="flex justify-between">
                                                    ...
                                                    <span className="font-bold">
                                                        {zone.longestBranchPipe.plantCount}
                                                    </span>
                                                </div>
                                                <div className="flex justify-between">
                    ...
                    {longestBranchPipeData && (
                        <div className="mt-6 rounded-lg bg-blue-900 p-4">
                            <h4 className="text-md mb-2 font-medium text-blue-200">
                                Longest Branch Pipe Analytics
                            </h4>
                            <div className="grid grid-cols-1 gap-4 text-sm md:grid-cols-3">
                                <div className="flex justify-between">
                                    <span>Zone:</span>
                                    <span className="font-bold">
                                        {longestBranchPipeData.zoneName}
                                    </span>
                                </div>
                                <div className="flex justify-between">
                                    <span>Length:</span>
                                    <span className="font-bold">
                                        {longestBranchPipeData.longestBranchPipe.length.toFixed(2)}{' '}
                                        m
                                    </span>
                                </div>
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-bold">
                                        {longestBranchPipeData.longestBranchPipe.plantCount}
                                    </span>
                                </div>
                            </div>
                        </div>
