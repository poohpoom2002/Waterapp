    ...
    const handleSaveToDatabase = async () => {
        ...
        try {
            ...
            if (fieldId && fieldId !== 'null' && fieldId !== 'undefined') {
                try {
                    const fieldCheck = await fetch(`/api/fields/${fieldId}`);
                    const fieldData = await fieldCheck.json();
            ...
            if (fieldId && fieldId !== 'null' && fieldId !== 'undefined') {
                response = await fetch(`/api/fields/${fieldId}`, {
                    method: 'PUT',
                    headers: {
                        'X-CSRF-TOKEN':
                            document
                ...
                response = await fetch('/api/save-field', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN':
                            document
