...
export const getPlotPlantingDensity = (cropValue: string): PlantingDensity => {
    return PLOT_PLANTING_DENSITIES[cropValue] || {
...
export const categorizeWaterIntensity = (litersPerSquareMeter: number): {
    category: 'low' | 'medium' | 'high' | 'very-high';
    description: string;
} => {
...
export const calculatePlotBasedWaterRequirements = (
    ...
    const totalEffectivePlantingArea = plotCalculations.reduce((sum, plot) => sum + plot.effectivePlantingArea, 0);
    ...
    const waterIntensities = plotCalculations.map(plot => plot.waterIntensity.litersPerSquareMeter);
    ...
    const utilizationRate = totalPlotArea > 0 ? (totalEffectivePlantingArea / totalPlotArea) * 100 : 0;
...
export const calculatePlotWaterCost = (litersPerDay: number, pricePerCubicMeter: number = 10): {
    daily: number;
    weekly: number;
    monthly: number;
    yearly: number;
...
export const recommendPlotBasedWaterStorage = (optimalDailyLiters: number, plotCount: number): {
    minimumSize: number;
...
export const compareCropWaterEfficiency = (plotCalculations: PlotWaterCalculation[]): {
    ...
    const averageEfficiency = efficiencies.reduce((sum, item) => sum + item.efficiency, 0) / efficiencies.length;
