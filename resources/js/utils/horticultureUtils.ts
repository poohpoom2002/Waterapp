...
export const generateBranchPipes = (
    ...
    for (let i = 0; i < numberOfBranches; i++) {
        const progress = numberOfBranches > 1 ? 0.05 + (i / (numberOfBranches - 1)) * 0.9 : 0.5;
        ...
        if (position) {
            ...
            ['left', 'right'].forEach((side, sideIndex) => {
                ...
                console.log(`🌿 ENHANCED Branch ${i + 1}-${side} - Complete Coverage:`, {
                    ...
                    areaUtilization:
                        ((branchLength / maxDistanceToBoundary) * 100).toFixed(1) + '%',
                });
                ...
                if (branchLength >= 15) {
                    ...
                    if (startInTargetArea && endInTargetArea) {
                        ...
                        console.log(
                            `✅ ENHANCED Branch ${i + 1}-${side}: ${plants.length} plants, length: ${branchLength.toFixed(2)}m, utilization: ${((branchLength / maxDistanceToBoundary) * 100).toFixed(1)}%`
                        );
                    } else {
                        console.log(
                            `⚠️ Branch ${i + 1}-${side} extends outside ${zoneName} boundary`
                        );
                    }
                }
            });
    ...
    console.log(
        `🌱 ENHANCED Generation complete: ${branchPipes.length} branches, ${totalPlants} plants in ${zoneName}`
    );
    console.log(
        `📐 ENHANCED: Branch pipes now cover 0.05-0.95 of submain length (90% coverage vs previous 80%)`
    );
    console.log(
        `🎯 ENHANCED: Submain end areas now have branch coverage with optimal plant spacing`
    );
...
const generateEnhancedFullAreaPlants = (
    ...
    try {
        ...
        for (let i = 0; i < numberOfPlants; i++) {
            ...
            if (position) {
                ...
                if (inZone && !inExclusion) {
                    ...
                    console.log(
                        `⚠️ Plant ${i + 1} excluded: inZone=${inZone}, inExclusion=${inExclusion}`
                    );
                }
            }
        }
        ...
        console.log(
            `✅ ENHANCED Plant distribution: ${plants.length} plants, coverage: ${((effectivePlantingLength / pipeLength) * 100).toFixed(1)}%`
        );
        return plants;
    } catch (error) {
...
const calculatePerpendicularDirection = (direction: {
    lat: number;
    lng: number;
}): { lat: number; lng: number } => {
    const perpendicular = {
        ...
        return data.zones.map((zone) => {
            ...
            const longestSubMainPipe =
                zoneSubMainPipes.length > 0
                    ? Math.max(...zoneSubMainPipes.map((pipe) => pipe.length))
                    : 0;
            const totalSubMainPipeLength = zoneSubMainPipes.reduce(
            ...
            const longestBranchPipe =
                allBranchPipes.length > 0
                    ? Math.max(...allBranchPipes.map((pipe) => pipe.length))
                    : 0;
            const totalBranchPipeLength = allBranchPipes.reduce(
                (sum, pipe) => sum + pipe.length,
                0
            );
...
export const calculateMainPipeAnalysis = (data: HorticultureProjectData): MainPipeAnalysis => {
    ...
    try {
        const longestMainPipe =
            data.mainPipes.length > 0 ? Math.max(...data.mainPipes.map((pipe) => pipe.length)) : 0;
        const totalMainPipeLength = data.mainPipes.reduce((sum, pipe) => sum + pipe.length, 0);

        const longestPipe =
            data.mainPipes.length > 0
    ...
    try {
        ...
        const actualTotalWaterNeed =
            data.plants && data.plants.length > 0
                ? data.plants.reduce((sum, plant) => sum + plant.plantData.waterNeed, 0)
                : 0;
        ...
        return {
            ...
            numberOfZones,
            totalPipeLength:
                zoneStatistics.reduce(
                    (sum, zone) => sum + zone.totalSubMainPipeLength + zone.totalBranchPipeLength,
                    0
                ) + mainPipeAnalysis.totalMainPipeLength,
...
export const createDefaultProject = (
    projectName: string = 'โครงการระบบน้ำสวนผลไม้'
...
if (typeof window !== 'undefined') {
    ...
    console.log(
        '🔧 ENHANCED: Reduced main pipe buffer from 50% to 30% for better plant distribution'
    );
